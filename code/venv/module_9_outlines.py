...
if __name__ == "__main__":
    ...
    while 1:
        ...
        if k == 27:
            ...
            cv.imshow("Sharr and Sobel outlines", grad)
            ...
            cv.imshow("Sobel outlines", grad)

        elif chr(k) == 'l':
            laplacian = cv.Laplacian(gray, ddepth=ddepth, ksize=3)
            abs_laplacian = cv.convertScaleAbs(laplacian)
            cv.imshow("Laplacian outlines", abs_laplacian)
